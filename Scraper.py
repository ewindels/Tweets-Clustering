...
def get_tweets(company, id_=0, mode='newest'):
...
def update_data(company, mode='newest'):
    """
    Supported modes are newest, oldest, and new_company
    """
    ...
    print('{:>5} tweets for {:<15}'.format(data_df.shape[0], company), end=' | ')

    if mode == 'newest':
        new_tweets = list()
        id_ = data_df['id'].max() + 1
        while True:
            tweets = get_tweets(company, id_, mode)
            tweets_filter = [tweet for tweet in tweets if (company.lower() not in tweet.user.name.lower())]
            if len(tweets_filter) > 0:
                new_tweets.extend(tweets_filter)
        ...
        id_ = 0
        tweets = get_tweets(company, id_, mode)
        new_tweets = [tweet for tweet in tweets if (company.lower() not in tweet.user.name.lower())]
        id_ = min([tweet.id for tweet in tweets]) - 1
        while True:
            mode = 'oldest'
            tweets = get_tweets(company, id_, mode)
